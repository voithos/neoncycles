...
var _previous_x = 0.0
var _previous_y = 0.0
var _previous_offset = Vector3(0, 0, 0)
...
func _process(delta):
    ...
    while _shake_trigger_accumulator >= _period:
        ...
        # other offset effects. We don't shake the Z axis.
        var new_offset = Vector3(x_component, y_component, 0)
        h_offset = x_component
        v_offset = y_component
        _previous_offset = new_offset
            
    _stopwatch += delta
    if _stopwatch > _duration:
        # Clear our offset once shaking is done.
        h_offset -= _previous_offset.x
        v_offset -= _previous_offset.y
        _shaking = false
